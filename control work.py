...
def cells(number_of_cells_: int) -> dict[int:str]:
    """
Функция генерирует словарь размера, равного входящему значению. Ключ равен номеру ячейки игрового поля.
    ...
    """
    cells = {}
    for key in range(1, (number_of_cells_ ** 2) + 1):
        cells[key] = 'None'
    return cells


def field(cells: dict, number_of_cells: int) -> str:  # функция рисующая поле
    """
...
def first_move(players_marker: list[tuple, tuple]) -> tuple[str, str]:
    """
...
def player_change(player: tuple[str, str], players_marker_list: list[tuple, tuple]) -> tuple[str, str]:
    """
...
def players_move(current_player: tuple[str, str], cells: dict) -> dict:
    """
Ф-я запрашивает у пользователя, какую ячейку в игровом поле он хочет занять. Ответ записывает в словарь.
    ...
    while True:
        answer = ''.join(input(
            f'Твой ход, {current_player[0]}! Выбери число, соответствующее ячейке, от 1 до {len(cells)}.\n=>').split())
...
def transpose_matrix(matrix: list[list, list, ...]) -> list[list, list, ...]:
    """
...
def check_line(matrix: list[list, list, ...]) -> bool:
    """
...
def diagonal(matrix: list[list, list, ...]) -> list[list, list, ...]:
    """
...
def check_win(cells: dict) -> bool:
    """
Проверяет текущее игровое поле на выигрышные комбинации ячеек.
    :param cells: Словарь, в котором сохранены ходы игроков. Ключ - номер ячейки поля, значение - маркер игрока или 'None'
    :return: результат проверки. True - совпадения найдены, False - нет.
    """

    def matrix(cells: dict) -> list[list, list, ...]:
        """
    ...
    return True if any([check_line(matrix(cells)), check_line(transpose_matrix(matrix(cells))),
                        check_line(diagonal(matrix(cells)))]) is True else False
...
Первый, выстроивший в ряд свои фигуры по вертикали, горизонтали или большой диагонали, выигрывает.\n")
    number_of_cells_ = number_of_cells()  # спрашиваем о размере поля, сохраняем в переменную
    players_marker_list = players_marker()  # спрашиваем о том, кто каким маркером играет, сохраняем в переменную
    first_move_ = first_move(players_marker_list)  # спрашиваем, кто ходит первый, сохраняем в переменную
    ...
    while True:
        ...
        if counter == len(cells_):
            print(field(cells_, number_of_cells_))
            print('Вышла ничья! Игра окончена.')
            break
